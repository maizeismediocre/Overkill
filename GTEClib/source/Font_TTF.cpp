...
bool CFont::LoadFont( string filename, int size)
{
	TTF_Font* font = NULL;
	char ascii[96];  // characters 32 to 127 only
	float advances[96]; // pixel offset from start
	

	string fname = "images/" + filename;
	
	cout << "font " << fname << " try loading\n";
	
	font = TTF_OpenFont( fname.c_str(), 32 );
	...
	cout << "font " << fname << " loaded\n";
	
	int minx,maxx,miny,maxy,advance;
	
	ascii[0]=32;
	TTF_GlyphMetrics(font,char(32),&minx,&maxx,&miny,&maxy,&advance);
	advances[0]=advance;
	
	for (int i=33; i <= 127; i++)
	{
		ascii[i-33]=i;
		TTF_GlyphMetrics(font,char(i),&minx,&maxx,&miny,&maxy,&advance);
		advances[i-32]=advances[i-33]+advance;
    ...
    * */
    ...
    SDL_Surface* fontimage = TTF_RenderText_Solid( font, "Hello GTEC3D", textColor ); //ascii
    
    cout << "Width: " << fontimage->w << "Height: " << fontimage->h <<  " bits:" << (int)fontimage->format->BitsPerPixel << endl;
    ...
    //glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE8, fontimage->w, fontimage->h, 0, GL_LUMINANCE8, GL_UNSIGNED_BYTE, fontimage->pixels);
    
    /*
    int overage = (Width) % 4;
    cout << "overage: " << overage << endl;
    int index=0;
    unsigned char* img = (unsigned char*)fontimage->pixels;
    for (int y=0; y < Height; y++)
    {
		for (int x=0; x < Width+4-overage; x++)
		{
	...
	unsigned char* img = (unsigned char*)fontimage->pixels;
	for (int i=0; i < Width*Height; i++) img[i]*=255;
	
	glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, fontimage->w, fontimage->h, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, fontimage->pixels);
